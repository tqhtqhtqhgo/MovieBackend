    ...
    conn.query(sqlStr,[phone],(error,result,field)=>{
      if (error){
        ...
        if (result[0]){
          ...
          res.json({success_code: 200,user_id:result[0].user_id});
        } else{
          //用户不存在
          ...
          conn.query(sqlStr,[phone,phone,avatarSrc,123456],(error,result,field)=>{
            if (error){
              ...
              res.json({success_code:200,user_id:result.insertId});
            }
          })
        }
      }
...
router.post('/api/pwdLogin',function(req,res){
  ...
  if (req.body.captcha.toLowerCase()!==req.session.captcha){
    res.json({error_code:1, message:'验证码不正确'})
//    console(captcha.toLowerCase());
//  console(req.session.captcha);
  } else{
...
router.post('/api/updateUserPwd',function(req,res){
  let {
    userId,
    userPwd,
  } = req.body;
  if (userId){
    ...
    conn.query(sqlStr,[userId],(error,result,field)=>{
      if (error){
        ...
        //更新数据库
        let sqlStr = 'UPDATE t_user SET password = ? WHERE user_id = ?;';
        conn.query(sqlStr,[userPwd,userId],(error,result,field)=>{
          if (error){
            res.json({error_code:1,message:'更新用户密码失败'});
          } else{
            res.json({success_code:200});
          }
        });
